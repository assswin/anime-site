...
function CustomCursor({ themeColor }) {
  ...
  useEffect(() => {
    ...
    const checkHoverState = (x, y) => {
      ...
      if (!el) return;
      const target = el.closest('a, button, .cursor-hover-target');
      setHovering(!!target);
      const deck = el.closest('.legends-deck-area');
      setIsDeck(!!deck);
    };
    ...
    const move = (e) => {
      ...
      if (cursorRef.current) {
        cursorRef.current.style.left = lastX - 10 + 'px'
        cursorRef.current.style.top = lastY - 10 + 'px'
        
        // If in deck area, determine side
        const xPercent = lastX / window.innerWidth
...
function AnimatedCounter({ value, label, icon }) {
  ...
  return (
    <motion.div
      ...
      className="glass-card rounded-2xl p-4 md:p-8 text-center flex flex-col items-center gap-2 md:gap-3 group hover:border-primary/40 transition-all duration-500"
    >
      <span className="text-3xl">{icon}</span>
      <span className="text-3xl md:text-5xl font-display font-bold bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent">
        {count}+
      </span>
