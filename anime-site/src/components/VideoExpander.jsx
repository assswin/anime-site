...
export default function VideoExpander({ character, onClose }) {
  ...
  const [isUiHidden, setIsUiHidden] = useState(false) // Toggle UI visibility
  ...
  useEffect(() => {
    if (character) {
      ...
      // Start loading the video now (preload="none" means it won't load until we ask)
      if (videoRef.current) {
      ...
      const timers = [
        ...
        setTimeout(() => {
          ...
          // Video will autoplay once canplay fires (see onCanPlay handler below)
        }, 1400),
      ]
  ...
  // Handle video canplay — only play when both ready and phase >= 3
  const handleVideoCanPlay = useCallback(() => {
    setVideoReady(true)
    if (phase >= 3 && document.visibilityState === 'visible' && videoRef.current) {
      videoRef.current.play().catch(() => {})
    }
  }, [phase])

  // If phase reaches 3 and video is already ready, play it
  useEffect(() => {
    if (phase >= 3 && videoReady && document.visibilityState === 'visible' && videoRef.current) {
      videoRef.current.play().catch(() => {})
    }
  }, [phase, videoReady])
  ...
  useEffect(() => {
    if (!character) return
    const dialog = dialogRef.current || overlayRef.current
    if (!dialog) return
      ...
      {character && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-[100vw] h-[100dvh] md:w-[90vw] md:h-[90vh] md:rounded-3xl overflow-hidden z-[9100]"
            style={{
          ...
          >
            {/* Video background */}
            <div ref={dialogRef} className="absolute inset-0 z-0 bg-[#050510]">
              <motion.video
                ref={videoRef}
                src={character.video}
                loop
                ...
                animate={{ 
                  opacity: phase >= 3 && videoReady ? 0.85 : 0, 
              ...
              {phase >= 3 && !videoReady && (
                <div className="absolute inset-0 flex items-center justify-center z-[1]">
                  <div className="w-full h-full animate-pulse bg-gradient-to-br from-[#050510] via-white/[0.03] to-[#050510]" />
                  <div className="absolute flex flex-col items-center gap-3">
                    <div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: `${character.themeColor}60`, borderTopColor: 'transparent' }} />
                    <span className="text-[10px] tracking-[0.3em] uppercase text-white/30 font-display">Loading</span>
                  </div>
                </div>
              )}
            ...
            {/* Content grid */}
            <motion.div 
              className="relative z-[2] w-full h-full flex flex-col md:flex-row items-center justify-center p-6 md:p-16 gap-8 cursor-pointer"
            ...
            >
              ...
              <motion.div
                ...
                className="w-[60%] sm:w-[50%] md:w-[30%] h-[35%] sm:h-[45%] md:h-[75%] relative flex-shrink-0"
              >
                <img
                  src={character.image}
                  alt={character.name}
              ...
              {/* Info panel */}
              <div className="flex flex-col items-center md:items-start justify-center gap-4 md:gap-6 text-center md:text-left mt-4 md:mt-0">
                <motion.h2
            ...
            <AnimatePresence>
              {isUiHidden && (
                ...
                >
                  <p className="text-white/50 text-xs tracking-widest font-display uppercase bg-black/40 px-4 py-2 rounded-full backdrop-blur-sm border border-white/10">
            ...
            <motion.button
              ...
              onClick={onClose}
              className="absolute top-4 right-4 md:top-6 md:right-6 z-[20] w-12 h-12 md:w-14 md:h-14 rounded-full glass-card flex items-center justify-center transition-all duration-300 group cursor-hover-target shadow-xl"
