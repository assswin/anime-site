  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
           <motion.div
             className="w-[90vw] max-w-[560px] sm:max-w-[600px] rounded-2xl overflow-hidden"
           ...
           >
            {/* Search input */}
             <div className="flex items-center gap-3 sm:px-5 sm:py-4 border-b border-white/[0.06]">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'ArrowDown') {
                    e.preventDefault()
                    setActiveIndex(prev => Math.min(prev + 1, filteredCharacters.length - 1))
                  } else if (e.key === 'ArrowUp') {
                    e.preventDefault()
                    setActiveIndex(prev => Math.max(prev - 1, 0))
                  } else if (e.key === 'Enter' && filteredCharacters.length > 0) {
                    e.preventDefault()
                    const char = filteredCharacters[activeIndex]
                    const originalIndex = characters.findIndex(c => c.id === char.id)
                    handleNavigate(char, originalIndex)
                  }
                }}
                ...
                aria-activedescendant={filteredCharacters.length > 0 ? `cmd-option-${filteredCharacters[activeIndex]?.id}` : undefined}
                aria-controls="cmd-results"
                role="combobox"
                aria-expanded={true}
                aria-autocomplete="list"
              />
                 <span className="text-[9px] sm:text-[10px] text-white/20 bg-white/[0.05] px-2 sm:px-3 py-1 sm:py-1.5 rounded font-mono">ESC</span>
            </div>

            {/* Results */}
            <div className="max-h-[45vh] overflow-y-auto py-2" role="listbox" id="cmd-results">
              {/* Character results */}
              {filteredCharacters.length > 0 ? (
                <>
                   <div className="px-4 sm:px-5 py-1 sm:py-2">
                     <span className="text-[9px] sm:text-[10px] tracking-[0.3em] uppercase text-white/20 font-display">Characters</span>
                   </div>
                   {filteredCharacters.map((character, i) => {
                     ...
                     return (
                       <motion.button
                         key={character.id}
                         id={`cmd-option-${character.id}`}
                         className={`w-full flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-2 sm:py-3 transition-colors text-left group ${i === activeIndex ? 'bg-white/[0.08]' : 'hover:bg-white/[0.04]'}`}
                       ...
                       >
                        ...
                        <div className="flex-1 min-w-0">
                           <div className="text-base sm:text-sm text-white/80 font-display tracking-wide group-hover:text-white transition-colors truncate">
                             {character.name}
                           </div>
                           <div className="text-[9px] sm:text-[10px] text-white/20 tracking-widest uppercase">
                             Power: {Math.round(Object.values(character.stats).reduce((a, b) => a + b, 0) / 5)}
                           </div>
                        </div>

                        {/* Theme color dot */}
                         <div className="w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full flex-shrink-0 opacity-60 group-hover:opacity-100 transition-opacity" style={{ backgroundColor: character.themeColor }} />

                        {/* Action hint */}
                         <span className="text-[9px] sm:text-[10px] text-white/10 group-hover:text-white/30 transition-colors flex-shrink-0">
                           ↵ Open
                         </span>
                      </motion.button>
                    )
