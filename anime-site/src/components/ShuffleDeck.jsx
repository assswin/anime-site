...
export default function ShuffleDeck({ characters, onSelectCharacter, onThemeChange, externalActiveIndex, onActiveIndexChange }) {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
     ...
     const handleMouseMove = (e) => {
       ...
       if (distance < 1.5) {
         // Active card pulls toward cursor (responsive max based on viewport)
         const vh = window.innerHeight * 0.01
         const maxOffsetX = Math.max(15, vh * 1.5) // Responsive max offset
         const maxOffsetY = Math.max(10, vh * 1)   // Responsive max offset
         setMagneticOffset({
           x: distX * maxOffsetX,
           y: distY * maxOffsetY,
         })
  ...
  return (
    <section 
      className="relative min-h-[85vh] md:min-h-[110vh] flex flex-col items-center justify-center py-10 md:py-20 overflow-visible legends-deck-area cursor-pointer"
    ...
    >
      ...
      <div 
        ref={deckRef}
        className="relative w-[280px] sm:w-[320px] md:w-full md:max-w-[45vw] lg:max-w-[35vw] flex items-center justify-center mt-8"
        style={{ 
          perspective: '1200px',
