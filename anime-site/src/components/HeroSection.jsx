      ...
      <div className="absolute inset-0 pointer-events-none bg-gradient-to-b from-transparent via-background/30 to-background z-[2]" />
       ...
       <div className="z-10 text-center flex flex-col items-center gap-6 px-4 sm:px-8">
        ...
        <motion.h1
          ...
          initial="hidden"
          animate="visible"
          className="text-4xl sm:text-5xl md:text-7xl lg:text-9xl font-cinematic font-extrabold tracking-[0.15em] flex flex-wrap justify-center"
          style={{ perspective: '1000px' }}
          aria-label={title}
        >
          {title.split(' ').map((word, wordIndex) => (
            <span key={wordIndex} className="inline-flex whitespace-nowrap mr-[0.5em] last:mr-0">
              {word.split('').map((char, charIndex) => (
                <motion.span
                  key={`${wordIndex}-${charIndex}`}
                  variants={letterVariants}
                  className="inline-block bg-gradient-to-b from-white via-fuchsia-200 to-primary bg-clip-text text-transparent drop-shadow-[0_0_25px_rgba(192,38,211,0.4)]"
                  aria-hidden="true"
                >
                  {char}
                </motion.span>
